...
import { seedNremtQuestions } from "./seed-nremt-questions";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/medications", async (req, res) => {
    try {
      ...
      const medications = await storage.getMedications(search as string, category as string, scope as string);
  ...
  app.get("/api/calculator-results", async (req, res) => {
    try {
      ...
      const defaultUserId = userId ? parseInt(userId as string) : 1;
      const results = await storage.getCalculatorResults(defaultUserId, type as string);
      res.json(results);
    } catch (error) {
  ...
  app.get("/api/learning-modules", async (req, res) => {
    try {
      ...
      const modules = await storage.getLearningModules(
        moduleNumber ? parseInt(moduleNumber as string) : undefined
      );
      res.json(modules);
    } catch (error) {
  ...
  app.get("/api/study-notes", async (req, res) => {
    try {
      ...
      const notes = await storage.getStudyNotes(
        chapterNumber ? parseInt(chapterNumber as string) : undefined
      );
      res.json(notes);
    } catch (error) {
  ...
  app.get("/api/flashcards", async (req, res) => {
    try {
      const chapterNumber = req.query.chapter ? parseInt(req.query.chapter as string) : undefined;
  ...
  app.get("/api/nremt-questions/:scope", async (req, res) => {
    try {
      ...
      const questions = await storage.getNremtQuestions(
        scope,
        contentArea as string,
  ...
  app.get("/api/nremt-sessions", async (req, res) => {
    try {
      ...
      const sessions = await storage.getNremtExamSessions(
        userId ? parseInt(userId as string) : undefined
      );
      res.json(sessions);
    } catch (error) {
  ...
  app.get("/api/dashboard/stats", async (req, res) => {
    try {
      ...
      const userProtocols = await storage.getProtocols(parseInt(userId as string));
